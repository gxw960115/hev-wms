...
jQuery(document).ready(function() {
	...
	jQuery('.rightTab .panel-close').click(function() {
	...
	$("#chatview").click(function() {
		$.messager.confirm("Info", "Are you sure to leave？", function (data) {
            if (data) {
	...
	//登录页tab切换样式
	$('.s-target').on('click', function(){
		var me = $(this);
		$(me.attr('data')).show().trigger('click');
	})
	...
	$("#serch-icon").click(function(){
		...
		$(".drop-left").click(function(){
			$(".leftpanelinner").hide();
			$(".mainpanel").css("margin-left","54px");
			$(".drop-right").show();
			$(".drop-right").css("display","block");
			});
	    $(".drop-right").click(function(){
			$(".leftpanelinner").show();
