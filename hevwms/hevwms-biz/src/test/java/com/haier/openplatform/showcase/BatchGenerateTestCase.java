...
public class BatchGenerateTestCase {
	 ...
	 * 单元测试需要继承的基类
	 */
	private static final String BASE_TEST_CASE_NAME = "com.haier.openplatform.showcase.BaseTestCase";
	/**
	 * 要生成单元测试的service的包名,请写到impl一级,例如com.haier.openplatform.showcase.test。service.impl
	 */
	private static final String PACKAGE_NAME = "com.haier.openplatform.showcase.test.service.impl";
	 ...
	 * @return
	 */
	public static List<Class<?>> generateTestCase(String packageName,
			boolean isRecursive) {
	 ...
	 * @throws IOException
	 */
	private static void generateTestCaseFile(Class<?> serviceClass,Map<String, List<String>> methodNameMap) throws IOException{
		...
		while(methodNameIt.hasNext()){
			...
			String fullFilePath = "src/test/java/" + pack.replaceAll("[.]", "/") + "_" + methodName + ".java";
			...
			if (files != null) {
				for (File file : files) {
					...
					if (file.isFile()) {
						...
						if (isRecursive) {
							String subPackagePath = getSubPackagePath(
