...
public class ExportOdsSoScanInfoListTemplet extends
		AbstractExcelExportTemplate<OdsSoScanInfoDTO> {

	private List<OdsSoScanInfoDTO> odsSoScanInfoList;
	
	public ExportOdsSoScanInfoListTemplet(List<OdsSoScanInfoDTO> odsPoScanInfoList) {
		this.odsSoScanInfoList = odsPoScanInfoList;
	...
	public String[][] getTitles() {
		return new String[][]{{"OGP/IGP No","Order Type","SO NO","SO Item NO","Plant","Location","Bin","Material No","Material Desp","Qty","Unit","Barcode","Old Barcode","Scanned By","Scanned Date"}};
	}
	...
	protected void buildBody(int sheetIndex) {
		...
		final int size = odsSoScanInfoList.size();
		...
		for(int i = 0; i < size; i++){
			...
			OdsSoScanInfoDTO info = odsSoScanInfoList.get(i);
			...
			createStyledCell(row,index++,info.getSoNo(),this.bodyRowStyle);
			createStyledCell(row,index++,info.getSoItemNo(),this.bodyRowStyle);
			...
			createStyledCell(row,index++,info.getOldBarcode(),this.bodyRowStyle);
