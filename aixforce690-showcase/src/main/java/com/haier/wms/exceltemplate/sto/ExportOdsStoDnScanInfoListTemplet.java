...
public class ExportOdsStoDnScanInfoListTemplet extends AbstractExcelExportTemplate<OdsStodnScanInfoDTO> {

	private List<OdsStodnScanInfoDTO> odsStoDnScanInfoList;

	public ExportOdsStoDnScanInfoListTemplet(List<OdsStodnScanInfoDTO> odsStoDnScanInfoList) {
		this.odsStoDnScanInfoList = odsStoDnScanInfoList;
		this.columnWidth = 4000;
	}
	
	@Override
	public String[] getSheetNames() {
		return new String[] {"ODS_STODN_SCAN_INFO"};
	}
	...
	public String[][] getTitles() {
		return new String[][]{{"Order NO","Order Type","STO NO","STO DN NO","STODN Item","bin","Barcode","Scanned Qty","Unit","Material No","Material Desp","Plant","Location","Scan Date"}};
	}
	...
	protected void buildBody(int sheetIndex) {
		...
		final int size = odsStoDnScanInfoList.size();
		...
		for(int i = 0; i < size; i++){
			...
			OdsStodnScanInfoDTO info = odsStoDnScanInfoList.get(i);
			...
			createStyledCell(row,index++,info.getStodnItem(),this.bodyRowStyle);
