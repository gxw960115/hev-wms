...
public class CasUtil {
	...
	public static String getServerName(HttpServletRequest request){
		...
		String serverName = request.getRequestURL().delete(request.getRequestURL().length() - request.getRequestURI().length(), request.getRequestURL().length()).toString(); 
		String[] serverNames = serverName.split(":");
        //如果是80端口访问,weblogic取得的serverName是带80的,这里把80去掉
        StringBuffer tmpServerName = new StringBuffer();
        for(String str : serverNames){
        	if("80".equals(str)){
        		continue;
        	}
        	tmpServerName.append(str).append(":");
        }
